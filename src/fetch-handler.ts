...
function resolveVersionedApiPathname(pathname: string): {
  ...
  return {
    normalizedPathname,
    versionedPathname: normalizedPathname,
  };
}
...
function isAccessAuthenticatedRequest(req: Request): boolean {
  const email = req.headers.get(ACCESS_EMAIL_HEADER);
...
function appendAccessVary(res: Response): Response {
  appendVaryHeader(res.headers, ACCESS_EMAIL_HEADER);
  return res;
}

function applyPrivateNoStore(res: Response, req?: Request): Response {
  if (!req || isAccessAuthenticatedRequest(req)) {
...
function shouldBypassPublicSharedCaching(req: Request): boolean {
  return hasTraceTokenHeaderValue(req);
}
...
function isPublicUiPath(url: URL): boolean {
  ...
  if (pathname === '/api/incidents') return true;
  if (pathname === '/api/maintenance-windows') return true;
  if (pathname === '/api/analytics/uptime') return true;
...
async function resolveTrace(req: Request, env: Env): Promise<Trace | null> {
  if (!isTruthyEnvFlag(req.headers.get('X-Uptimer-Trace'))) return null;
...
async function handlePublicStatus(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  const { applyStatusCacheHeaders, readStaleStatusSnapshotJson } =
    await import('./snapshots/public-status');
  const now = Math.floor(Date.now() / 1000);
  const includeHiddenMonitors = isAccessAuthenticatedRequest(req);
  const hasTraceTokenHeader = hasTraceTokenHeaderValue(req);
  const shouldBypassSharedCaching = hasTraceTokenHeader;
  const trace = await resolveTrace(req, env);
  if (trace) {
...
export async function handleFetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  if (resolvedApiPath) {
    ...
    // 规范化重复斜杠后，将旧版 `/api/*` 路径重定向到版本化 API。
  ...
  try {
    ...
    if (hotPathname === '/api/analytics/uptime') {
  ...
  // SPA fallback：对于非 API 请求的 HTML 导航，返回 index.html
  if (!resolvedApiPath && normalizedRequest.method === 'GET') {
    const accept = normalizedRequest.headers.get('Accept') ?? '';
    if (accept.includes('text/html') && !accept.includes('application/json') && env.ASSETS) {
      ...
      if (indexRes.status === 200) {
        ...
        try {
          ...
          const siteTitle = (results?.[0]?.value ?? '').trim();
          if (siteTitle) {
            const escaped = siteTitle
              .replace(/&/g, '&amp;')
              .replace(/</g, '&lt;')
              .replace(/>/g, '&gt;')
              .replace(/"/g, '&quot;')
            ...
            html = html
              ...
              .replace('<meta property="og:site_name" content="" />', `<meta property="og:site_name" content="${escaped}" />`)
              .replace('<meta name="twitter:title" content="" />', `<meta name="twitter:title" content="${escaped}" />`);
          }
        } catch {
          // 读取失败时保持空白，静默处理
        }
  ...
  // 其余路由通过懒加载导入，以确保冷启动 CPU 资源集中在热路径上。
