...
publicHotRoutes.get('/status', async (c) => {
  ...
  const now = Math.floor(Date.now() / 1000);
  const includeHiddenMonitors = isAccessAuthenticated(c.req);
  const trace = new Trace(
    resolveTraceOptions({
      header: (name) => c.req.header(name),
      env: c.env as unknown as Record<string, unknown>,
  ...
  trace.setLabel('auth_present', includeHiddenMonitors);
  ...
  if (snapshot) {
    c.header('Content-Type', 'application/json; charset=utf-8');
    const res = appendAccessVary(c.body(snapshot.bodyJson));
  ...
  try {
    ...
    const res = appendAccessVary(c.json(payload));
...
// 其他所有内容都放在延迟导入之后，使冷启动的 CPU 资源集中在首页/状态页的热路径上。
