...
function shouldSkipFragmentByTime(
...
export function parseMonitorRuntimeUpdateFragmentRows(
  ...
  for (const row of rows) {
    if (shouldSkipFragmentByTime(row, opts)) {
      staleCount += 1;
      continue;
    }
...
function parseEnvelopeFragment<T extends { generated_at: number }>(
  rows: readonly PublicSnapshotFragmentRow[],
  schema: z.ZodType<T, any, any>, // eslint-disable-line @typescript-eslint/no-explicit-any
...
function parseMonitorFragmentRows<T extends { id: number }>(
  ...
  for (const row of rows) {
    ...
    if (previousGeneratedAt === undefined || row.generated_at >= previousGeneratedAt) {
      latestByMonitorId.set(monitorId, parsed.data as T);
...
export function parseHomepageEnvelopeFragmentRows(
  ...
  return parseEnvelopeFragment<PublicHomepageEnvelopeFragment>(rows, homepageEnvelopeFragmentSchema);
}

export function parseStatusEnvelopeFragmentRows(
  rows: readonly PublicSnapshotFragmentRow[],
): PublicSnapshotEnvelopeReadResult<PublicStatusEnvelopeFragment> | null {
  return parseEnvelopeFragment<PublicStatusEnvelopeFragment>(rows, statusEnvelopeFragmentSchema);
}
...
export function parseHomepageMonitorFragmentRows(
  ...
  return parseMonitorFragmentRows<HomepageMonitorFragment>(rows, homepageMonitorCardSchema, opts);
}
...
export function parseStatusMonitorFragmentRows(
  ...
  return parseMonitorFragmentRows<StatusMonitorFragment>(rows, statusMonitorFragmentSchema, opts);
}
