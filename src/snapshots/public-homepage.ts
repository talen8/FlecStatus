...
import { isRecord, parseJsonText } from './shared';
...
function readStoredHomepageSnapshotData(value: unknown): PublicHomepageResponse | null {
  ...
  if (artifact.success) {
    ...
    return readStoredHomepageSnapshotData(parseJsonText(artifact.data.snapshot_json)?.value ?? null);
  }
...
function readStoredHomepageSnapshotRender(value: unknown): PublicHomepageRenderArtifact | null {
  ...
  if (artifact.success) {
    ...
    const snapshot = readStoredHomepageSnapshotData(parseJsonText(artifact.data.snapshot_json)?.value ?? null);
    if (!snapshot) {
      return null;
    }
    return {
...
async function readSnapshotRowsByPriority(
  ...
  return homepageRow ? [homepageRow] : [];
}
...
function normalizeHomepagePayloadBodyJson(bodyJson: string): string | null {
  ...
  const data = readStoredHomepageSnapshotData(parsed.value);
...
function normalizeHomepageArtifactBodyJson(bodyJson: string): string | null {
  ...
  const directArtifact = publicHomepageStoredRenderArtifactSchema.safeParse(parsed.value);
  if (directArtifact.success) {
    if (!('snapshot' in directArtifact.data) && !readStoredHomepageSnapshotData(parseJsonText(directArtifact.data.snapshot_json)?.value ?? null)) {
      return null;
    }
  ...
  if (
    ...
    !readStoredHomepageSnapshotData(parseJsonText(legacyArtifact.data.snapshot_json)?.value ?? null)
  ) {
    return null;
  }
...
export async function readHomepageSnapshot(
  ...
  const snapshot = readSnapshotValueFromRows({
    ...
    normalize: (bodyJson) => {
      ...
      return readStoredHomepageSnapshotData(parsed.value);
    },
  });
...
export async function readHomepageSnapshotArtifact(
  ...
  const snapshot = readSnapshotValueFromRows({
    ...
    normalize: (bodyJson) => {
      ...
      return readStoredHomepageSnapshotRender(parsed.value);
    },
  });
  return snapshot ? { data: snapshot.value, age: snapshot.age } : null;
}
...
async function releaseRefreshLease(
  ...
  try {
    await Trace.timed(trace, spanName, async () =>
...
export function prepareHomepageSnapshotWrite(
  ...
  const bodyJson = Trace.timedSync(trace, 'homepage_write_stringify', () => JSON.stringify(payload));
  if (trace?.enabled) {
    trace.setLabel('homepage_payload_monitors', payload.monitors.length);
    trace.setLabel('homepage_payload_bytes', bodyJson.length);
  }
...
export async function writeHomepageSnapshot(
  ...
  const writeResult = await Trace.timed(trace, 'homepage_write_run', async () =>
...
export async function writeHomepageArtifactSnapshot(
  ...
  const render = Trace.timedSync(trace, 'homepage_artifact_write_render', () =>
    buildHomepageRenderArtifact(payload),
  );
  const renderBodyJson = Trace.timedSync(trace, 'homepage_artifact_write_stringify', () =>
    JSON.stringify(render),
  );

  const writeResult = await Trace.timed(trace, 'homepage_artifact_write_run', async () =>
...
export async function refreshPublicHomepageSnapshot(opts: {
  ...
  const computed = await Trace.timed(opts.trace, 'homepage_refresh_compute', async () =>
    await opts.compute(),
  );
  const payload = Trace.timedSync(opts.trace, 'homepage_refresh_validate', () =>
...
export async function refreshPublicHomepageArtifactSnapshot(opts: {
  ...
  const computed = await Trace.timed(
    opts.trace,
    'homepage_artifact_refresh_compute',
    async () => await opts.compute(),
  );
  const payload = Trace.timedSync(opts.trace, 'homepage_artifact_refresh_validate', () =>
    toHomepageSnapshotPayload(computed),
  );
...
export async function refreshPublicHomepageSnapshotIfNeeded(opts: {
  ...
  if (!opts.force) {
    const generatedAt = await Trace.timed(
      opts.trace,
  ...
  const acquired = await Trace.timed(opts.trace, 'homepage_refresh_lease', async () =>
  ...
  try {
    if (!opts.force) {
      const latestGeneratedAt = await Trace.timed(
        opts.trace,
    ...
    const wrote = await Trace.timed(opts.trace, 'homepage_refresh_write', async () =>
...
export async function refreshPublicHomepageArtifactSnapshotIfNeeded(opts: {
  ...
  const acquired = await Trace.timed(opts.trace, 'homepage_artifact_refresh_lease', async () =>
    await acquireLease(opts.db, REFRESH_LOCK_NAME, opts.now, REFRESH_LOCK_LEASE_SECONDS),
  );
  if (!acquired) {
    return false;
  ...
  try {
    ...
    const wrote = await Trace.timed(opts.trace, 'homepage_artifact_refresh_write', async () =>
