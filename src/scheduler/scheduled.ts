...
import type { HttpResponseMatchMode, MonitorStatus } from '../db/schema';

import type { Env } from '../env';
import { isTruthyEnvFlag } from '../internal/env-utils';
...
import { acquireLease, releaseLease, acquireLeaseBatch, releaseLeaseBatch } from './lock';
...
function shouldLogScheduledRefresh(env: Env): boolean {
  const raw = envRecord(env).UPTIMER_SCHEDULED_REFRESH_LOGS;
  if (typeof raw !== 'string') {
    return true;
  }
...
function shouldRefreshRuntimeFragmentsViaService(env: Env): boolean {
  return isTruthyEnvFlag(envRecord(env).UPTIMER_SCHEDULED_RUNTIME_FRAGMENT_REFRESH);
}

function shouldUseScheduledRuntimeFragmentPipeline(env: Env): boolean {
  const r = envRecord(env);
  return (
    Boolean(env.SELF) &&
    shouldRefreshRuntimeFragmentsViaService(env) &&
    isTruthyEnvFlag(r.UPTIMER_PUBLIC_MONITOR_UPDATE_FRAGMENT_WRITES)
  );
}

function shouldSplitInternalCheckBatchFragmentWrites(env: Env): boolean {
  return isTruthyEnvFlag(envRecord(env).UPTIMER_INTERNAL_CHECK_BATCH_FRAGMENT_WRITE_SPLIT);
}

function shouldSeedScheduledShardedFragments(env: Env): boolean {
  return Boolean(env.SELF) && isTruthyEnvFlag(envRecord(env).UPTIMER_SHARDED_FRAGMENT_SEED);
}

function shouldAssembleScheduledShardedSnapshots(env: Env): boolean {
  return Boolean(env.SELF) && isTruthyEnvFlag(envRecord(env).UPTIMER_SHARDED_ASSEMBLER);
}
...
function shouldSkipScheduledHomepageRefreshForShardedSnapshots(env: Env): boolean {
  ...
  return (
    Boolean(env.SELF) &&
    isTruthyEnvFlag(r.UPTIMER_SCHEDULED_SHARDED_SKIP_HOMEPAGE_REFRESH) &&
...
function shouldUseScheduledShardedContinuation(env: Env): boolean {
  ...
  return (
    Boolean(env.SELF) &&
    isTruthyEnvFlag(r.UPTIMER_SCHEDULED_SHARDED_CONTINUATION) &&
    (shouldSeedScheduledShardedFragments(env) || shouldAssembleScheduledShardedSnapshots(env))
  );
}
...
async function fetchSelfWithTimeout(
  ...
  /* v8 ignore next -- 父级中止传播由集成租约丢失测试覆盖，而非定时任务测试覆盖。 */
...
async function writeRuntimeUpdateFragmentsViaService(
  ...
  const res = await fetchSelfWithTimeout(
    env,
    new Request('http://internal/internal/write/runtime-update-fragments', {
      method: 'POST',
      headers: {
...
async function refreshRuntimeFragmentsViaService(env: Env): Promise<void> {
  const res = await fetchSelfWithTimeout(
    env,
    new Request('http://internal/internal/refresh/runtime-fragments', {
      method: 'POST',
      headers: {
...
function readShardedPublicSnapshotAssemblyMode(env: Env): ShardedPublicSnapshotAssemblyMode {
  const raw = envRecord(env).UPTIMER_SHARDED_ASSEMBLER_MODE;
...
async function seedShardedPublicSnapshotPartViaService(
  ...
  const res = await fetchSelfWithTimeout(
    env,
    new Request('http://internal/internal/seed/sharded-public-snapshot', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json; charset=utf-8',
      },
...
async function assembleShardedPublicSnapshotViaService(
  ...
  const res = await fetchSelfWithTimeout(
    env,
    new Request('http://internal/internal/assemble/sharded-public-snapshot', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json; charset=utf-8',
      },
...
async function startShardedPublicSnapshotContinuationViaService(
  ...
  const res = await fetchSelfWithTimeout(
    env,
    new Request('http://internal/internal/continue/sharded-public-snapshot', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json; charset=utf-8',
      },
...
async function refreshHomepageSnapshotViaService(
  ...
  const res = await fetchSelfWithTimeout(
    env,
    new Request('http://internal/internal/refresh/homepage', {
      method: 'POST',
      headers,
      body: runtimeUpdates
        ? JSON.stringify({
...
async function runScheduledCheckBatchViaService(
  ...
  const res = await fetchSelfWithTimeout(
    env,
    new Request('http://internal/internal/scheduled/check-batch', {
      method: 'POST',
      headers,
      body: JSON.stringify({
        ids: context.ids,
...
const DUE_MONITOR_FROM = `
  FROM monitors m
  LEFT JOIN monitor_state s ON s.monitor_id = m.id`;

const LIST_DUE_MONITORS_SQL = `
  SELECT${DUE_MONITOR_SELECT}
${DUE_MONITOR_FROM}
  WHERE m.is_active = 1
...
async function claimMonitorExecutionLeases(
  ...
  const entries = ids.map((id) => ({
    name: buildMonitorExecutionLockName(checkedAt, id),
    now,
    leaseSeconds: MONITOR_EXECUTION_LOCK_LEASE_SECONDS,
  }));
  const acquired = await acquireLeaseBatch(db, entries);

  const leases: MonitorExecutionLease[] = [];
  for (let i = 0; i < ids.length; i++) {
    if (!acquired[i]) continue;
    const id = ids[i];
    if (id === undefined) throw new Error(`ids[${i}] is undefined`);
    const entry = entries[i];
    if (!entry) throw new Error(`entries[${i}] is undefined`);
    claimedIds.push(id);
    leases.push({ id, name: entry.name, expiresAt });
  }
...
export async function runExclusivePersistedMonitorBatch(opts: {
  ...
  try {
    ...
    const releaseBatchLeases = async () => {
      if (claimedMonitorLeases.length > 0) {
        await releaseLeaseBatch(
          opts.db,
          claimedMonitorLeases.map((lease) => ({
            name: lease.name,
            expiresAt: lease.expiresAt,
          })),
        ).catch((err) => {
          console.warn('scheduled: failed to release monitor execution leases', err);
        });
      }
...
export async function runPersistedMonitorBatch(opts: {
  ...
  // Promise.allSettled 会保留输入顺序，因此这里保持现有的监控项排序。
