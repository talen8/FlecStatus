import { parseDbJson, webhookChannelConfigSchema } from '../db/schemas';
...
function dispatchMaintenanceWindowEvent(
  env: Env,
  ...
  const eventType = `maintenance.${event}`;
  const eventKey = `maintenance:${window.id}:${event}:${event === 'started' ? window.starts_at : window.ends_at}`;
  const payload = {
    event: eventType,
    event_id: eventKey,
    timestamp: event === 'started' ? window.starts_at : window.ends_at,
    maintenance: maintenanceWindowRowToPayload(window, monitorIds),
  };
  ...
  notify.ctx.waitUntil(
    getWebhookDispatchModule()
      ...
      .catch((err) => {
        console.error(`notify: failed to dispatch ${eventType}`, err);
      }),
  );
}
...
export async function emitMaintenanceWindowNotifications(
  ...
  for (const w of started) {
    ...
    dispatchMaintenanceWindowEvent(env, notify, w, monitorIdsByWindowId.get(w.id) ?? [], 'started', channelsForEvent);
  }

  for (const w of ended) {
    const channelsForEvent = notify.channels.filter((c) => c.created_at <= w.ends_at);
    if (channelsForEvent.length === 0) continue;
    dispatchMaintenanceWindowEvent(env, notify, w, monitorIdsByWindowId.get(w.id) ?? [], 'ended', channelsForEvent);
  }
}
