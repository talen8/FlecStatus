...
export class Trace {
  ...
  static timed<T>(trace: Trace | null | undefined, name: string, fn: () => Promise<T>): Promise<T> {
    return trace?.timingsEnabled ? trace.timeAsync(name, fn) : fn();
  }

  static timedSync<T>(trace: Trace | null | undefined, name: string, fn: () => T): T {
    return trace?.timingsEnabled ? trace.time(name, fn) : fn();
  }
