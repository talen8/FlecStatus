...
export async function buildPublicMonitorCards(
  ...
  if (ids.length > 0) {
    ...
    // 这样可以避免新部署后（rangeStart 可能在今天 00:00 之后）直到下一次每日汇总前，
    // 显示空白的可用性条带。
        ...
        ? Promise.resolve(
            new Map(
              ids.map((id) => {
                ...
                if (!entry) throw new Error(`Runtime entry not found for monitor ${id}`);
                return [id, runtimeEntryToHeartbeats(entry)] as [number, PublicStatusResponse['monitors'][number]['heartbeats']];
              }),
            ),
          )
