...
type CachedHomepageScheduledFastGuardState = {
  settings: {
    site_title: string;
...
type HomepageGuardCacheReadResult =
  | {
      source: 'db_cache';
...
const homepageGuardStateJsonSchema = z.object({
  ...
  guard_state: z.object({
    settings: z.object({
      ...
      site_nav_links: z.array(z.object({ label: z.string(), url: z.string() })).default([]),
...
async function bumpHomepageGuardVersions(
  db: D1Database,
