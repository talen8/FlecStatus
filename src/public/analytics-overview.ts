...
const PUBLIC_ANALYTICS_OVERVIEW_SNAPSHOT_KEY = 'analytics-overview';
const PUBLIC_ANALYTICS_OVERVIEW_SNAPSHOT_VERSION = 1;
...
type AnalyticsOverviewRange = '30d' | '90d';

type PublicAnalyticsOverviewTotals = {
  total_sec: number;
  downtime_sec: number;
  unknown_sec: number;
  uptime_sec: number;
...
type PublicAnalyticsOverviewSnapshotEntry = {
  monitor_id: number;
...
const publicAnalyticsOverviewSnapshotSchema = z.custom<PublicAnalyticsOverviewSnapshot>(
  isSnapshot,
...
async function writePublicAnalyticsOverviewSnapshot(
  db: D1Database,
  snapshot: PublicAnalyticsOverviewSnapshot,
  now: number,
): Promise<void> {
...
async function buildPublicAnalyticsOverviewSnapshot(opts: {
  db: D1Database;
  fullDayEndAt: number;
  now: number;
