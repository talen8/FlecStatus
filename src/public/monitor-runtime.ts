...
type CompactMonitorRuntimeUpdate = [
  monitor_id: number,
  interval_sec: number,
  created_at: number,
  checked_at: number,
...
const publicMonitorRuntimeSnapshotSchema = z.object({
...
const publicMonitorRuntimeTotalsSnapshotSchema = z.custom<PublicMonitorRuntimeTotalsSnapshot>(
...
const runtimeSnapshotMonitorIdsBySnapshot = new WeakMap<PublicMonitorRuntimeSnapshot, Set<number>>();
const runtimeTotalsSnapshotMonitorIdsBySnapshot = new WeakMap<PublicMonitorRuntimeTotalsSnapshot, Set<number>>();
const runtimeSnapshotEntryMapBySnapshot = new WeakMap<PublicMonitorRuntimeSnapshot, Map<number, PublicMonitorRuntimeEntry>>();
const runtimeTotalsSnapshotEntryMapBySnapshot = new WeakMap<PublicMonitorRuntimeTotalsSnapshot, Map<number, PublicMonitorRuntimeTotalsEntry>>();
const runtimeEntryHeartbeatsByEntry = new WeakMap<
  PublicMonitorRuntimeEntry,
  MonitorRuntimeHeartbeat[]
>();
let runtimeSnapshotCacheGlobal: RuntimeSnapshotCacheEntry | null = null;
let runtimeTotalsSnapshotCacheGlobal: RuntimeTotalsSnapshotCacheEntry | null = null;
...
type RuntimeSnapshotCacheEntry = CachedSnapshotEntry<PublicMonitorRuntimeSnapshot>;
type RuntimeTotalsSnapshotCacheEntry = CachedSnapshotEntry<PublicMonitorRuntimeTotalsSnapshot>;
...
function readCachedSnapshot<S extends { monitors: readonly unknown[] }>(
  cache: WeakMap<D1Database, CachedSnapshotEntry<S>>,
  db: D1Database,
  generatedAt: number,
  updatedAt: number,
  rawBodyJson: string,
  ...
  return cached.generatedAt === generatedAt &&
    ...
    ? (cached.snapshot as S)
    : null;
}

function writeCachedSnapshot<S extends { monitors: readonly unknown[] }>(
  cache: WeakMap<D1Database, CachedSnapshotEntry<S>>,
  db: D1Database,
  generatedAt: number,
  updatedAt: number,
  ...
  cache.set(db, { generatedAt, updatedAt, rawBodyJson, snapshot } as CachedSnapshotEntry<S>);
  return snapshot;
}

function readCachedSnapshotGlobal<S>(
  globalCache: CachedSnapshotEntry<S> | null,
  generatedAt: number,
  updatedAt: number,
  rawBodyJson: string,
): S | null {
  if (!globalCache) return null;
  return globalCache.generatedAt === generatedAt &&
    globalCache.updatedAt === updatedAt &&
    globalCache.rawBodyJson === rawBodyJson
    ? globalCache.snapshot
    : null;
}

function readSnapshotIndex<
  E extends { monitor_id: number },
  S extends { monitors: readonly E[] },
>(
  snapshotCache: WeakMap<S, Map<number, E>>,
  idsCache: WeakMap<S, Set<number>>,
  snapshot: S,
): { entryMap: ReadonlyMap<number, E>; ids: ReadonlySet<number> } {
  let entryMap = snapshotCache.get(snapshot);
  let ids = idsCache.get(snapshot);
  if (!entryMap) {
    entryMap = new Map<number, E>();
    for (const entry of snapshot.monitors) {
      entryMap.set(entry.monitor_id, entry);
    }
    snapshotCache.set(snapshot, entryMap);
  }
  if (!ids) {
    ids = new Set<number>();
    for (const entry of snapshot.monitors) {
      ids.add(entry.monitor_id);
    }
    idsCache.set(snapshot, ids);
  }
  return { entryMap, ids };
}

function isValidStoredSnapshot(
  stored: { generatedAt: number; snapshot: { day_start_at: number } },
  now: number,
  maxAgeSeconds: number,
): boolean {
  if (stored.generatedAt > now + FUTURE_SNAPSHOT_TOLERANCE_SECONDS) return false;
  if (Math.max(0, now - stored.generatedAt) > maxAgeSeconds) return false;
  if (stored.snapshot.day_start_at !== utcDayStart(now)) return false;
  return true;
}
...
async function readStoredMonitorRuntimeSnapshot(
  ...
  try {
    ...
    const cached = readCachedSnapshot(runtimeSnapshotCacheByDb, db, row.generated_at, updatedAt, row.body_json);
    if (cached) return { generatedAt: row.generated_at, snapshot: cached };

    const globalCached = readCachedSnapshotGlobal(runtimeSnapshotCacheGlobal, row.generated_at, updatedAt, row.body_json);
    if (globalCached) {
      return { generatedAt: row.generated_at, snapshot: writeCachedSnapshot(runtimeSnapshotCacheByDb, db, row.generated_at, updatedAt, row.body_json, globalCached) };
    }

    const parsed = publicMonitorRuntimeSnapshotSchema.safeParse(JSON.parse(row.body_json));
    if (!parsed.success) {
      console.warn('monitor runtime: invalid snapshot payload', parsed.error.message);
      return null;
    }

    runtimeSnapshotCacheGlobal = { generatedAt: row.generated_at, updatedAt, rawBodyJson: row.body_json, snapshot: parsed.data };
    return { generatedAt: row.generated_at, snapshot: writeCachedSnapshot(runtimeSnapshotCacheByDb, db, row.generated_at, updatedAt, row.body_json, parsed.data) };
  } catch (err) {
...
async function readStoredMonitorRuntimeTotalsSnapshot(
  ...
  try {
    ...
    const updatedAt = toSnapshotUpdatedAt(row);
    const cached = readCachedSnapshot(runtimeTotalsSnapshotCacheByDb, db, row.generated_at, updatedAt, row.body_json);
    if (cached) return { generatedAt: row.generated_at, snapshot: cached };

    const globalCached = readCachedSnapshotGlobal(runtimeTotalsSnapshotCacheGlobal, row.generated_at, updatedAt, row.body_json);
    if (globalCached) {
      return { generatedAt: row.generated_at, snapshot: writeCachedSnapshot(runtimeTotalsSnapshotCacheByDb, db, row.generated_at, updatedAt, row.body_json, globalCached) };
    }

    const parsed = publicMonitorRuntimeTotalsSnapshotSchema.safeParse(JSON.parse(row.body_json));
    if (!parsed.success) {
      console.warn('monitor runtime totals: invalid snapshot payload', parsed.error.message);
      return await readStoredMonitorRuntimeTotalsSnapshotFromFullSnapshot(db);
    }

    runtimeTotalsSnapshotCacheGlobal = { generatedAt: row.generated_at, updatedAt, rawBodyJson: row.body_json, snapshot: parsed.data };
    return { generatedAt: row.generated_at, snapshot: writeCachedSnapshot(runtimeTotalsSnapshotCacheByDb, db, row.generated_at, updatedAt, row.body_json, parsed.data) };
  } catch (err) {
...
export async function readPublicMonitorRuntimeSnapshot(
  ...
  return stored && isValidStoredSnapshot(stored, now, maxAgeSeconds) ? stored.snapshot : null;
}
...
export async function writePublicMonitorRuntimeSnapshot(
  ...
  writeCachedSnapshot(runtimeSnapshotCacheByDb, db, snapshot.generated_at, writeNow, bodyJson, snapshot);
  runtimeSnapshotCacheGlobal = { generatedAt: snapshot.generated_at, updatedAt: writeNow, rawBodyJson: bodyJson, snapshot };
  writeCachedSnapshot(runtimeTotalsSnapshotCacheByDb, db, totalsSnapshot.generated_at, writeNow, totalsBodyJson, totalsSnapshot);
  runtimeTotalsSnapshotCacheGlobal = { generatedAt: totalsSnapshot.generated_at, updatedAt: writeNow, rawBodyJson: totalsBodyJson, snapshot: totalsSnapshot };
...
export function snapshotHasMonitorIds(
  ...
  if (monitorIds.length > snapshot.monitors.length) return false;

  const { ids } = readSnapshotIndex(runtimeSnapshotEntryMapBySnapshot, runtimeSnapshotMonitorIdsBySnapshot, snapshot);
  for (const monitorId of monitorIds) {
    if (!ids.has(monitorId)) return false;
  }
  return true;
}
...
export function toMonitorRuntimeEntryMap(
  ...
  return readSnapshotIndex(runtimeSnapshotEntryMapBySnapshot, runtimeSnapshotMonitorIdsBySnapshot, snapshot).entryMap;
}
...
export function toMonitorRuntimeTotalsEntryMap(
  ...
  return readSnapshotIndex(runtimeTotalsSnapshotEntryMapBySnapshot, runtimeTotalsSnapshotMonitorIdsBySnapshot, snapshot).entryMap;
}
...
export function applyMonitorRuntimeUpdates(
  ...
  const sourceById = readSnapshotIndex(runtimeSnapshotEntryMapBySnapshot, runtimeSnapshotMonitorIdsBySnapshot, snapshot).entryMap;
...
export async function refreshPublicMonitorRuntimeSnapshot(opts: {
  ...
  const snapshotMonitorIds = readSnapshotIndex(runtimeSnapshotEntryMapBySnapshot, runtimeSnapshotMonitorIdsBySnapshot, snapshot).ids;
